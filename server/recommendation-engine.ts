/**
 * server/recommendation-engine.ts — Dype, regelbaserte anbefalinger basert på repo-analyse.
 */
import type { Recommendation, ProjectType } from '../packages/core';
import type { TreeItem } from './file-analyzer';
import type { FileTreeMetrics } from './types';

// ─── Konstanter ──────────────────────────────────────────────────────────────

export const TEST_DIRS = ['__tests__', 'test', 'tests', 'spec', 'specs', 'e2e', '__test__'];
...
export function detectTestsFromTree(tree: TreeItem[], packageJsonContent: string | null): boolean {
...
interface GenerateDeepRecommendationsParams {
...
export function generateDeepRecommendations(params: GenerateDeepRecommendationsParams): Recommendation[] {
  const {
    repo,
    isPublic,
    rootFiles,
  ...
  if (!readmeContent) {
    recommendations.push({
      ...
      description:
        'Repositoryet mangler README. Legg til installasjonsinstruksjoner, brukseksempler og bidragsguide.',
    ...
    recommendations.push({
      ...
      title: 'Utvid README',
      description:
        'README er veldig kort. Legg til installasjonsinstruksjoner, brukseksempler og skjermbilder.',
  ...
  if (!hasWorkflows) {
    recommendations.push({
      ...
      description:
        'Repositoryet mangler GitHub Actions workflows. Legg til automatisk testing og bygg ved push/PR.',
      marketOpportunity:
        'CI/CD reduserer bugrate, øker trygghet for bidragsytere og signaliserer prosjektmodenhet.',
    });
  }
  ...
  if (!hasTests) {
    ...
    recommendations.push({
      ...
      description:
        'Ingen tester funnet. Legg til enhetstester for å sikre kvalitet og gjøre det tryggere å bidra.',
      marketOpportunity:
        'Testdekning er kritisk for biblioteker og API-er og øker bidragsyternes trygghet.',
    });
  }
  ...
  if (fileTreeMetrics) {
    const hasLinterConfig = Object.keys(configFiles).some(
      (f) => f.includes('eslint') || f.includes('.pylintrc') || f === 'tslint.json',
    );
    const hasFormatterConfig = Object.keys(configFiles).some(
      (f) => f.includes('prettier') || f === '.editorconfig',
    );
    ...
    if (!hasLinterConfig && fileTreeMetrics.byCategory.code > 5 && projectType !== 'docs') {
      recommendations.push({
        ...
        description:
          'Prosjektet mangler en linter-konfigurasjon (ESLint, Pylint, etc.). Linting opprettholder konsistent kodekvalitet.',
    ...
    if (!hasFormatterConfig && fileTreeMetrics.byCategory.code > 10 && projectType !== 'docs') {
      recommendations.push({
        ...
        description:
          'Prosjektet mangler Prettier eller EditorConfig. Automatisk formattering sparer tid og reduserer PR-støy.',
    ...
    if (!hasDependabotConfig && fileTreeMetrics.byCategory.code > 0 && isPublic) {
      recommendations.push({
        ...
        description:
          'Konfigurer Dependabot for automatiske avhengighetsoppdateringer og sikkerhetsvarsler.',
        marketOpportunity:
          'Automatiske avhengighetsoppdateringer reduserer sikkerhetsrisiko og vedlikeholdsbyrde.',
      });
    }
        ...
        try {
          ...
          if (deps['dotenv'] || deps['@nestjs/config'] || deps['env-var']) {
            recommendations.push({
              ...
              description:
                'Prosjektet bruker miljøvariabler, men mangler .env.example. Dokumenter påkrevde variabler for enklere oppsett.',
              marketOpportunity:
                'God onboarding-dokumentasjon er avgjørende for utviklertilfredshet.',
            });
          }
        } catch {
          // ignore
    ...
    if (!hasChangelogDoc && isPublic && projectType === 'library') {
      recommendations.push({
        ...
        description:
          'Biblioteker bør ha en endringslogg. Bruk Keep a Changelog-format eller automatiser med Changesets/conventional-changelog.',
        marketOpportunity:
          'Tydelig endringslogg gir brukere oversikt over nye funksjoner og breaking changes.',
      });
    }
    ...
    if (!hasDockerConfig && projectType === 'api') {
      recommendations.push({
        ...
        description:
          'API-et mangler Dockerfile. Containerisering forenkler deployment og sikrer konsistent kjøremiljø.',
    ...
    if (
      fileTreeMetrics.sourceDirs.length === 0 &&
      fileTreeMetrics.byCategory.code > 10 &&
      fileTreeMetrics.maxDepth < 3
    ) {
      recommendations.push({
        type: 'architecture',
        priority: 'medium',
        ...
        description:
          'Kodefilene ligger spredt uten en tydelig mappestruktur (src/, lib/). God kodeorganisering letter navigasjon og vedlikehold.',
    ...
    if (buildGradleContent && buildGradleContent.includes('minSdkVersion')) {
      ...
      if (minSdk && minSdk < 24) {
        recommendations.push({
          ...
          marketOpportunity:
            'Høyere minSdkVersion gir tilgang til moderne API-er og reduserer vedlikeholdsbyrde.',
        });
      }
    }

    if (!buildGradleContent?.includes('kotlin') && (repo.language as string) !== 'Kotlin') {
      recommendations.push({
        type: 'architecture',
        priority: 'medium',
        title: 'Migrer til Kotlin',
        description:
          'Kotlin er Googles foretrukne språk for Android og tilbyr bedre DSL, coroutines og null-safety.',
        marketOpportunity:
          'Kotlin-adopsjon øker bidragsyterbas og forenkler bruk av Jetpack Compose.',
      });
    }
  }
      ...
      try {
        ...
        if (!deps['typescript'] && !deps['@types/node']) {
          recommendations.push({
            ...
            description:
              'Prosjektet bruker ikke TypeScript. TypeScript øker kodesikkerhet og gir bedre IDE-støtte.',
            marketOpportunity:
              'TypeScript er industristandard for seriøse web-prosjekter og øker bidragsyterbas.',
          });
        }
      } catch {
        // ignore
      ...
      try {
        ...
        if (!pkg.version || pkg.version === '0.0.0' || pkg.version === '0.0.1') {
          recommendations.push({
            ...
            description:
              'Biblioteket har ikke en stabil versjon (1.x.x). Vurder å publisere til npm med semantisk versjonering.',
            marketOpportunity:
              'Stabil versjon signaliserer modenhet og gjør det tryggere å ta i bruk.',
          });
        }
        if (!pkg.description) {
          recommendations.push({
  ...
  if (recentCommits.length === 0) {
    recommendations.push({
      ...
      description:
        'Repositoryet har hatt null aktivitet siste 30 dager. Vurder å sette opp en vedlikeholdsplan.',
